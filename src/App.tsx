...
export default function App() {
  ...
  const [serverUrl, setServerUrl] = useState((import.meta as any).env.VITE_SERVER_URL || 'https://juke-2.onrender.com');
  ...
  const api = useCallback({
    post: async (path: string, data: any) => {
      if (Capacitor.isNativePlatform()) {
        ...
        let finalUrl = path;
        if (path === '/api/proxy/check') finalUrl = `${cleanBase}/api/machine/check`;
        else if (path === '/api/proxy/pix/create') finalUrl = `${cleanBase}/api/machine/pix/create`;
        else if (path === '/api/proxy/pix/status') finalUrl = `${cleanBase}/api/machine/pix/status`;
  ...
  const getFullUrl = (path: string) => {
    ...
    if (!Capacitor.isNativePlatform() && path.startsWith('/api/proxy')) {
      return path;
    }
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      if (path === '/api/proxy/check') return `${cleanBase}/api/machine/check`;
      if (path === '/api/proxy/pix/create') return `${cleanBase}/api/machine/pix/create`;
      if (path === '/api/proxy/pix/status') return `${cleanBase}/api/machine/pix/status`;
  ...
  useEffect(() => {
    if (isConfigLoaded) {
      saveConfig({ serverUrl, token, credits, revenue: totalRevenue });
    }
  }, [serverUrl, token, credits, totalRevenue, isConfigLoaded]);
  ...
  const syncWithServer = useCallback(async () => {
    ...
    setSyncError('');
    try {
      const url = getFullUrl('/api/proxy/check');
      logDebug(`Request URL: ${url}`);
      logDebug(`Payload: token=${token.substring(0, 4)}..., hwid=${hwid.substring(0, 8)}...`);
      
      const response = await api.post(url, { token, hwid });
      ...
      if (data.ok) {
        ...
        setSyncError(data.error || 'Servidor recusou a conexão (Token inválido?)');
      }
    } catch (err: any) {
      const errorDetail = err.response?.data?.error || err.response?.data?.message || err.message;
      const status = err.response?.status;
      logDebug(`Sync Connection Error: ${errorDetail} (Status: ${status})`);
      
      if (err.code === 'ECONNABORTED') {
        setSyncError('Tempo esgotado. O servidor Render pode estar "dormindo", tente novamente em 1 minuto.');
      } else if (status === 404) {
        setSyncError('Erro 404: Endpoint não encontrado no servidor Render.');
      } else if (status === 405) {
        setSyncError('Erro 405: Método POST não permitido. Verifique a URL do servidor.');
      } else if (err.message === 'Network Error') {
        setSyncError('Erro de Rede: Possível problema de CORS ou falta de internet no celular.');
      } else {
        setSyncError(`Erro de conexão: ${errorDetail}. Verifique se o Token está correto.`);
      }
    } finally {
      setIsLoading(false);
    }
  }, [serverUrl, token, hwid, screen, isMobile, logDebug]);
  ...
  useEffect(() => {
    ...
    const checkPayment = async () => {
      ...
      try {
        const url = getFullUrl('/api/proxy/pix/status');
  ...
  const generatePix = async () => {
    setIsLoading(true);
    try {
      const url = getFullUrl('/api/proxy/pix/create');
      const response = await api.post(url, { 
        token, 
        amount: 1.00, 
        credits: 2 
      });
      // Algumas APIs retornam os dados diretamente sem o wrapper "ok"
      const data = response.data;
      if (data.ok || data.copy_paste || data.qr_code || data.id) {
        setPixData(data);
      } else {
        console.error("Resposta inválida do PIX:", data);
      }
    } catch (err) {
      ...
      <AnimatePresence mode="wait">
        ...
        {screen === 'locked' && (
          <motion.div 
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-red-950"
          >
            <AlertTriangle className="w-24 h-24 text-white mb-6" />
            <h1 className="text-4xl font-bold mb-4">SISTEMA BLOQUEADO</h1>
            <p className="text-red-200 text-lg mb-8">Sua licença expirou ou é inválida.<br/>Favor entrar em contato com o suporte.</p>
            <p className="text-xs opacity-50 mb-12">ID: {hwid}</p>
            <button 
          ...
          >
            ...
            <div className="space-y-6">
              <section className="space-y-4">
                <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-widest pl-1">Conexão do Servidor</h3>
                <div className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-zinc-600 uppercase ml-1">URL do Backend</label>
                    <input 
                      type="text" 
                      value={serverUrl} 
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-zinc-600 uppercase ml-1">Chave do Sistema (Token)</label>
                    <input 
                      type="text" 
                      value={token} 
